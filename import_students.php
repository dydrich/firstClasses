...
$sel_students .= "WHERE rb_alunni.id_classe = rb_classi.id_classe AND anno_corso = 5 AND id_alunno = alunno AND anno = {$year} AND quadrimestre = 2 AND (materia <> 30 AND materia <> 40) AND id_alunno NOT IN (SELECT id_archivio FROM rb_fc_alunni WHERE id_archivio IS NOT NULL) ";
